...
ПРАВИЛА: макс 120 слов, один вопрос в конце, используй цифры и ROI. После 3-го ответа предложи встречу: «Предлагаю 30-минутную встречу — разберём вашу задачу и покажу расчёт ROI. Бесплатно. Когда удобно?»
...
- В конце — призыв к действию: написать в @Alex_AXENTRA или ссылка на бот
...
ЗАПРЕЩЕНО: корпоративный канцелярит, «в современном мире», «не секрет что», списки из 10 пунктов, символы * _ ` # (никакого markdown-форматирования).

Верни ТОЛЬКО текст поста, без пояснений, кавычек и форматирования."""
...
def channel_keyboard():
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("⚡️ AI Assistant", url="https://t.me/ITAxentra_bot/app")
    ]])
...
async def generate_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /post [тема] — генерирует пост через Claude и показывает превью."""
    ...
    if not topic:
        await update.message.reply_text(
            ...
            "/post почему CRM не внедряется\n"
            "/post кибербезопасность для малого бизнеса\n"
            "/post ROI от внедрения BI аналитики"
        )
        return
    ...
    try:
        ...
        preview = (
            ...
            f"━━━━━━━━━━━━━━━\n"
            f"📊 Символов: {len(post_text)} | Тема: <i>{html.escape(topic)}</i>"
        )
        await msg.edit_text(preview, parse_mode="HTML", reply_markup=keyboard)
...
async def post_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Кнопки: Опубликовать / Перегенерировать / Отмена."""
    ...
    if action == "post_regen":
        ...
        if not pending:
            await query.edit_message_text("❌ Пост не найден. Создайте новый через /post")
            return

        await query.edit_message_text(f"⏳ Генерирую новый вариант: {pending['topic']}...")
        ...
        try:
            ...
            preview = (
                f"📋 <b>Превью поста (новый вариант):</b>\n\n"
                f"{html.escape(post_text)}\n\n"
                f"━━━━━━━━━━━━━━━\n"
                f"📊 Символов: {len(post_text)} | Тема: <i>{html.escape(pending['topic'])}</i>"
            )
...
async def publish_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Прямая публикация — отправьте текст/фото/видео в бот."""
    user_id = update.effective_user.id
    if user_id not in ADMIN_IDS:
        return
    ...
    try:
        ...
        if msg.photo:
            ...
            await msg.reply_text("✅ Пост опубликован в канале!")
    except Exception as e:
...
async def handle_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if session["step"] == "name":
        ...
        await update.message.reply_text(
            ...
            f"📱 Оставьте номер телефона — пришлю персональные рекомендации:"
        )
        return
